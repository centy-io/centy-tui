# Hide sidebar when not in a project context
...
- Views requiring a project (issues, prs, docs, assets, config) are already hidden via getVisibleSidebarViews() in src/types/views.ts
...
- src/App.tsx - Conditionally render the Sidebar based on state.selectedProjectPath
- src/components/layout/Sidebar.tsx - May need adjustments for the conditional rendering
