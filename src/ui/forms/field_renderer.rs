...
use crate::state::forms::FormField;
use ratatui::{
    layout::Rect,
    style::{Color, Style},
    text::{Line, Span},
...
/// Draw a form field using the FormField domain object
pub fn draw_field(frame: &mut Frame, area: Rect, field: &FormField, is_active: bool) {
    ...
    let value_str = if display_value.is_empty() && !is_active {
        "(empty)".to_string()
    } else {
        display_value
    };
...
/// Draw a form field using raw values (for backward compatibility during migration)
pub fn draw_field_raw(
    frame: &mut Frame,
    area: Rect,
    label: &str,
    value: &str,
